...
class rmap(pix):
    ...
    def pad(self, nxp, nyp):
        ...
        ret.map[ int((nyp-self.ny)/2):int((nyp+self.ny)/2), int((nxp-self.nx)/2):int((nxp+self.nx)/2) ] = self.map
        return ret
...
class rfft(pix):
    def __init__(self, nx, dx, fft=None, ny=None, dy=None):
        ...
        if fft is None:
            fft = np.zeros( (self.ny, int(self.nx/2+1)), dtype=np.complex )
        self.fft = fft

        assert( (self.ny, int(self.nx/2+1)) == self.fft.shape )
    ...
    def get_cfft( self ):
        ...
        fft[:,0:int(int(self.nx/2+1))] = self.fft[:,:]
        fft[0,int(self.nx/2+1):]  = np.conj(self.fft[0,1:int(self.nx/2)][::-1])
        fft[1:,int(self.nx/2+1):]  = np.conj(self.fft[1:,1:int(self.nx/2)][::-1,::-1])
